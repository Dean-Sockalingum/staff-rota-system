...
def senior_management_dashboard(request):
    ...
    if mandatory_courses.exists():
        # Pre-fetch all training records in one query to avoid N+1 problem
        training_records_prefetch = Prefetch(
            'training_records',
            queryset=TrainingRecord.objects.filter(
                course__in=mandatory_courses
            ).select_related('course').order_by('staff_member', 'course', '-completion_date'),
            to_attr='prefetched_training_records'
        )
        
        for home in care_homes:
            # Prefetch training records for all staff in this home
            home_staff = User.objects.filter(
                unit__care_home=home,
                is_active=True
            ).prefetch_related(training_records_prefetch).distinct()
            ...
            if total_staff > 0:
                # Build a lookup dict for latest records per staff/course
                for staff in home_staff:
                    # Group prefetched records by course
                    records_by_course = {}
                    for record in getattr(staff, 'prefetched_training_records', []):
                        course_id = record.course_id
                        if course_id not in records_by_course:
                            records_by_course[course_id] = record
                    
                    # Check compliance for each mandatory course
                    for course in mandatory_courses:
                        latest_record = records_by_course.get(course.id)
