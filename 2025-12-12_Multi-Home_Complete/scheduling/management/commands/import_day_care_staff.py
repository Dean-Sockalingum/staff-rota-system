...
class Command(BaseCommand):
    help = 'Import day shift care staff (SCW and SCA) with correct team distribution from spreadsheet'

    def handle(self, *args, **options):
        # Day shift care staff data from spreadsheet - CORRECTED TEAM ASSIGNMENTS
        # Team 1 (A): 27 staff (9 SCW + 18 SCA)
        # Team 2 (B): 26 staff (9 SCW + 17 SCA)
        # Team 3 (C): 26 staff (9 SCW + 17 SCA)
        # Total: 79 staff (27 SCW + 52 SCA)
        # Hours: 40 staff × 24hr (2 shifts/week) + 39 staff × 35hr (3 shifts/week)
        day_care_staff = [
            # ===== Team 2 (B) - 26 staff total =====
            # SCW: 9 staff (3×35hr + 6×24hr)
            ...
            {'role': 'SCA', 'first': 'Taylor', 'surname': 'Henderson', 'sap': 'SCA1071', 'hours': 35, 'shifts_per_week': 3, 'unit': 'ORANGE', 'team': 'B'},
            
            # ===== Team 3 (C) - 26 staff total =====
            # SCW: 9 staff (3×35hr + 6×24hr)
            ...
            {'role': 'SCA', 'first': 'Sebastian', 'surname': 'Ross', 'sap': 'SCA1070', 'hours': 35, 'shifts_per_week': 3, 'unit': 'BLUE', 'team': 'C'},
            
            # ===== Team 1 (A) - 27 staff total =====
            # SCW: 9 staff (3×35hr + 6×24hr)
        ...
        with transaction.atomic():
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f'\n✅ Day Care Staff Import Complete (CORRECTED FROM SPREADSHEET)!\n'
            ...
            self.stdout.write('\n--- Summary by Team and Role ---')
            for team in ['A', 'B', 'C']:
                scw = User.objects.filter(team=team, role__name='SCW', is_staff=False).count()
                sca = User.objects.filter(team=team, role__name='SCA', is_staff=False).count()
                total = scw + sca
                self.stdout.write(f'Team {team}: {scw} SCW + {sca} SCA = {total} total')
            
            # Summary by hours
            self.stdout.write('\n--- Summary by Hours ---')
            hours_24 = User.objects.filter(role__name__in=['SCW', 'SCA'], shifts_per_week_override=2, is_staff=False).count()
            hours_35 = User.objects.filter(role__name__in=['SCW', 'SCA'], shifts_per_week_override=3, is_staff=False).count()
            self.stdout.write(f'24hr (2 shifts/week): {hours_24} staff')
            self.stdout.write(f'35hr (3 shifts/week): {hours_35} staff')
            self.stdout.write(f'\nExpected: 40×24hr + 39×35hr = 79 total')
