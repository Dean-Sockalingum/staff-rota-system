...
# SAP-to-home unit mapping (replace with your actual mapping)
SAP_TO_UNIT = {
    # This mapping is now handled dynamically using the rota pattern and SAP-to-pattern mapping from the rota command
}

class Command(BaseCommand):
    help = 'Assign missing home units to SCW/SCA day staff using SAP-to-unit mapping'
    ...
    def handle(self, *args, **options):
        ...
        staff_to_pattern_sap = {
            ...
            "SCA1018": "SCAD3A03",  # Assign to Dementia
            # TEAM A SCA 2-shift
        ...
        missing_staff = User.objects.filter(role__name__in=['SCW', 'SCA'], home_unit__isnull=True, shift_preference__contains='DAY')
        updated = 0
        not_found = []
        for staff in missing_staff:
            sap = staff.sap
            pattern_key = staff_to_pattern_sap.get(sap)
            if pattern_key and pattern_key in rota_patterns:
                team, unit_name, *_ = rota_patterns[pattern_key]
                try:
                    unit = Unit.objects.get(name=unit_name)
                    staff.home_unit = unit
                    staff.save(update_fields=['home_unit'])
                    updated += 1
                    self.stdout.write(f'Assigned {unit_name} to {sap}')
                except Unit.DoesNotExist:
                    not_found.append((sap, unit_name))
                    self.stdout.write(f'Unit not found for {sap}: {unit_name}')
            else:
                not_found.append((sap, None))
                self.stdout.write(f'No mapping for SAP: {sap}')
        self.stdout.write(self.style.SUCCESS(f'Updated {updated} staff home units.'))
        if not_found:
            self.stdout.write(self.style.WARNING('Staff not updated:'))
            for sap, unit_name in not_found:
                self.stdout.write(f'  SAP: {sap}, Unit: {unit_name}')
