...
def training_compliance_dashboard(request):
    """Management view of training compliance across all homes - OPTIMIZED"""
    ...
    try:
        ...
        # Get care homes - limit to selected home if provided
        if selected_home:
            care_homes = CareHome.objects.filter(is_active=True, name=selected_home)
        else:
        ...
        for home in care_homes:
            # Get active staff IDs for this home (just IDs, not full objects)
            home_staff_ids = list(User.objects.filter(
                unit__care_home=home,
                is_active=True
            ).values_list('id', flat=True).distinct())
            
            total_staff = len(home_staff_ids)
            
            # OPTIMIZATION: Only fetch training records, compute summaries in Python
            # This avoids the massive iteration over 814 staff objects
            all_training_records = {}
            for record in TrainingRecord.objects.filter(
                staff_member_id__in=home_staff_ids
            ).select_related('course').only(
                'id', 'staff_member_id', 'course_id', 'completion_date', 
                'expiry_date', 'course__name', 'course__validity_period_months'
            ).order_by('staff_member_id', 'course_id', '-completion_date'):
                key = (record.staff_member_id, record.course_id)
                if key not in all_training_records:
                    all_training_records[key] = record
            
            # Calculate compliance for each mandatory course - SUMMARY ONLY
            course_compliance = []
            for course in mandatory_courses:
                compliant_count = 0
                expiring_count = 0
                ...
                for staff_id in home_staff_ids:
                    latest_record = all_training_records.get((staff_id, course.pk))
                ...
                course_compliance.append({
                    ...
                    # Remove staff lists - they cause timeout with 814 staff
                    'compliant_staff': [],
                    'expiring_staff': [],
                    'expired_staff': [],
                    'missing_staff': [],
        ...
        context = {
            ...
            'care_homes': CareHome.objects.filter(is_active=True).order_by('name'),  # For filter dropdown
        ...
        <h1>Training Compliance Dashboard Error</h1>
        <h2>Error: {str(e)}</h2>
        <h3>Type: {type(e).__name__}</h3>
        <h3>Traceback:</h3>
        <pre>{traceback.format_exc()}</pre>
        <p><a href='/manager/'>Return to Manager Dashboard</a></p>
        </body></html>
        """
        logger.exception(f'Error in training_compliance_dashboard: {str(e)}')
        return HttpResponse(error_details, status=500)
...
def add_staff_training_record(request):
    ...
    if request.method == 'POST':
        ...
        try:
            staff_member = User.objects.get(pk=staff_id)
...
def training_breakdown_report(request):
    ...
    # Prefetch all training records to avoid N+1 queries
    all_records = prefetch_training_records(staff_list, mandatory_courses)
    ...
    for staff in staff_list:
        ...
        for course in mandatory_courses:
            latest_record = all_records.get((staff.pk, course.pk))
