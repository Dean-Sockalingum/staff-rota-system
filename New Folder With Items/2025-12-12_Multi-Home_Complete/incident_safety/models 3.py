...
class CorrectivePreventiveAction(models.Model):
    """
    CAPA (Corrective and Preventive Actions) tracking for incidents and RCAs.
    Ensures actions are assigned, tracked, and verified for effectiveness.
    """
    ...
    action_owner = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        null=True,
        related_name='capa_actions_owned'
    )
    supporting_staff = models.ManyToManyField(
        User,
        blank=True,
        related_name='capa_actions_supporting'
    )
    ...
    verified_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name='capa_actions_verified'
    )
    ...
    created_by = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        null=True,
        related_name='capa_actions_created'
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.reference_number:
            ...
            last_capa = CorrectivePreventiveAction.objects.filter(
                reference_number__startswith=f'CAPA-{year}-'
            ).order_by('-reference_number').first()
            
            if last_capa:
                last_num = int(last_capa.reference_number.split('-')[-1])
                new_num = last_num + 1
            else:
                new_num = 1
            
            self.reference_number = f'CAPA-{year}-{new_num:03d}'
