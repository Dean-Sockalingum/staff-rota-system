...
def training_compliance_dashboard(request):
    ...
    try:
        ...
        for home in care_homes:
            ...
            # Prefetch all training records for this home to avoid N+1 queries
            all_training_records = {}
            for record in TrainingRecord.objects.filter(
                staff_member__in=home_staff
            ).select_related('staff_member', 'course').order_by('staff_member_id', 'course_id', '-completion_date'):
                key = (record.staff_member_id, record.course_id)
            ...
            for course in mandatory_courses:
                ...
                for staff in home_staff:
                    try:
                        # Get latest training record from prefetched data
                        latest_record = all_training_records.get((staff.pk, course.pk))
                        ...
                        logger.exception(f"Error processing training record for staff {staff.pk} ({staff}), course {course.pk} ({course}): {e}")
                        # Skip this staff member and continue
                        missing_count += 1
                        continue
