...
from .utils_dashboard_optimization import (
    get_optimized_coverage_summary,
    get_optimized_unit_coverage,
    get_optimized_staffing_by_unit,
    get_optimized_role_distribution,
    get_weekly_shift_trends
)
...
def manager_dashboard(request):
    ...
    # Coverage summary - OPTIMIZED using aggregation queries
    rota_start = datetime(2026, 1, 4).date()
    rota_end = rota_start + timedelta(weeks=12) - timedelta(days=1)
    
    # Use optimized query instead of per-day/per-unit iteration
    # This reduces 1000s of queries to a single aggregate query
    coverage_data = get_optimized_coverage_summary(rota_start, rota_end, user_home)
    
    # Get active units for template context
    units_qs = Unit.objects.filter(is_active=True)
    if user_home:
        units_qs = units_qs.filter(care_home=user_home)
    units = list(units_qs.order_by('name'))
    
    # Convert coverage_data to list format for template compatibility
    coverage_table = []
    all_days = [(rota_start + timedelta(days=i)) for i in range((rota_end-rota_start).days+1)]
    for day in all_days:
        row = {'date': day}
        for unit in units:
            unit_data = coverage_data.get(day, {}).get(unit.name, {})
            row[f'{unit.name}_day_sscw'] = unit_data.get('day_sscw', 0)
            row[f'{unit.name}_day_care'] = unit_data.get('day_care', 0)
            row[f'{unit.name}_night_sscw'] = unit_data.get('night_sscw', 0)
            row[f'{unit.name}_night_care'] = unit_data.get('night_care', 0)
    ...
    for i in range(7):
        ...
        day_care = date_shifts.filter(
            ...
            user__role__name__in=['SCW', 'SCA', 'SCWN', 'SCAN', 'SSCW', 'SSCWN']
        ).count()
    ...
    response = render(request, 'scheduling/manager_dashboard.html', context)
...
def rota_view(request):
    ...
    print(f"\n=== ROTA VIEW DEBUG ===")
    print(f"Selected home: {selected_home}")
    print(f"Selected unit: {selected_unit}")
    print(f"Total shifts in query: {len(all_shifts_list)}")
    if all_shifts_list:
        print(f"Sample shift: {all_shifts_list[0].user.full_name if all_shifts_list[0].user else 'NO USER'} - {all_shifts_list[0].unit.name} - {all_shifts_list[0].date}")
    
    # Day roles: HOS, SM, OM, SSCW, SCW, SCA
    # Night roles: SSCWN, SCWN, SCAN
    # Note: SSCW and SSCWN are handled separately as supernumerary, not in care roles
    day_care_roles = {'SCW', 'SCA'}
    night_care_roles = {'SCWN', 'SCAN'}
    ...
    for i in range(7):
        ...
        # Management staff (HOS, IDI, SM, OM) - always at top with pink background
...
class ReportGenerator:
    ...
    @staticmethod
    def generate_shift_coverage_report(date_str=None, care_home_name=None):
        """Generate shift coverage report for a specific date with staff names, optionally filtered by care home"""
        ...
        shifts_qs = Shift.objects.filter(date=target_date).select_related('unit', 'unit__care_home', 'shift_type', 'user', 'user__role')
        ...
        if care_home_name:
            ...
            db_name = care_home_map.get(care_home_name.lower(), care_home_name.upper())
            shifts_qs = shifts_qs.filter(unit__care_home__name=db_name)
        
        shifts = list(shifts_qs)
        
        # Coverage by unit with staff names
        if care_home_name:
            # Only show units for this care home
            units = Unit.objects.filter(is_active=True, care_home__name__icontains=care_home_name.replace(' ', '_'))
        else:
            units = Unit.objects.filter(is_active=True)
        ...
        for unit in units:
            unit_shifts = [s for s in shifts if s.unit == unit]
            day_shifts = [s for s in unit_shifts if 'DAY' in s.shift_type.name.upper() or 'LONG' in s.shift_type.name.upper()]
            night_shifts = [s for s in unit_shifts if 'NIGHT' in s.shift_type.name.upper()]
            
            if len(unit_shifts) > 0:  # Only show units with shifts
                by_unit[unit.name] = {
                    'day': len(day_shifts),
                    'night': len(night_shifts),
                    'total': len(unit_shifts)
                }
                
                # Get staff names with roles
                staff_by_unit[unit.name] = {
                    'day_staff': [f"{s.user.full_name} ({s.user.role.name if s.user.role else 'No Role'}) - {s.shift_type.get_name_display()}" for s in day_shifts if s.user],
                    'night_staff': [f"{s.user.full_name} ({s.user.role.name if s.user.role else 'No Role'}) - {s.shift_type.get_name_display()}" for s in night_shifts if s.user],
                }
        ...
        for shift in shifts:
            if shift.shift_type:
                ...
                by_shift_type[shift_name] = by_shift_type.get(shift_name, 0) + 1
                if shift_name not in staff_by_shift_type:
                    staff_by_shift_type[shift_name] = []
                if shift.user:
                    staff_by_shift_type[shift_name].append(f"{shift.user.full_name} ({shift.user.role.name if shift.user.role else 'No Role'})")
        
        # Build summary
        if care_home_name:
            summary = f"**{care_home_name.title()} on {target_date.strftime('%A, %d %B %Y')}:**\n{len(shifts)} staff scheduled across {len(by_unit)} units"
        else:
            summary = f"On {target_date.strftime('%A, %d %B %Y')}: {len(shifts)} shifts scheduled across {len(by_unit)} units."
...
"""
    ...
    def interpret_query(query):
        ...
        # Care Home-Specific Staffing/Coverage Queries (check BEFORE generic coverage)
        # Pattern: "[home name] staffing/shifts/coverage/working [today/tonight]"
        care_home_patterns = ['orchard grove', 'meadowburn', 'hawthorn house', 'riverside', 'victoria gardens']
        is_care_home_query = any(home in query_lower for home in care_home_patterns)
        is_staffing_query = any(word in query_lower for word in ['staffing', 'staff', 'working', 'shifts', 'coverage', 'rota', 'schedule', 'on duty'])
        
        if is_care_home_query and is_staffing_query:
            # Extract care home name
            care_home_name = None
            for home in care_home_patterns:
            ...
            return {
                'type': 'shift_coverage',
                'data': ReportGenerator.generate_shift_coverage_report(date_str, care_home_name)
            }
        
        # Coverage queries (generic - only if NOT care-home-specific)
...
@require_http_methods(["POST"])
@csrf_exempt  # Allow unauthenticated access for demo/testing
def ai_assistant_api(request):
    """
    API endpoint for AI assistant queries with enhanced report generation
    NOTE: Authentication temporarily disabled for demo purposes
...
def home_dashboard(request, home_slug=None):
    ...
    if permission_level in ['FULL', 'MOST']:
        ...
        if permission_level == 'MOST':
            pending_qs = pending_qs.filter(user__role__name__in=['SCW', 'SCA', 'SCWN', 'SCAN', 'SSCW', 'SSCWN'])
...
@login_required
def compliance_dashboard(request):
    """Compliance monitoring dashboard - Updated for new models and senior leadership access"""
    from .models import StaffCertification, RegulatoryCheck, AuditTrail, CareHome
    from django.utils import timezone
    from datetime import timedelta
    
    today = timezone.now().date()
    
    # Senior leadership (HOS, IDI, SM, OM) see all homes, others see their assigned home
    if request.user.can_view_all_homes:
        # Senior leadership - portfolio-wide view
        care_homes = CareHome.objects.all()
        expiring_certifications = StaffCertification.objects.filter(
            status='EXPIRING_SOON'
        ).select_related('staff_member', 'staff_member__unit', 'staff_member__unit__care_home')[:10]
        
        expired_certifications_count = StaffCertification.objects.filter(
            status='EXPIRED'
        ).count()
        
        recent_checks = RegulatoryCheck.objects.all().order_by('-check_date')[:10]
        ...
        # Regular staff - home-specific view
        user_care_home = request.user.assigned_care_home
        if not user_care_home:
            # User has no assigned home - show empty dashboard
            context = {
                'compliance_score': 0,
                ...
                'no_home_assigned': True,
            }
            return render(request, 'scheduling/compliance_dashboard_new.html', context)
        
        care_homes = [user_care_home]
        expiring_certifications = StaffCertification.objects.filter(
            staff_member__unit__care_home=user_care_home,
            status='EXPIRING_SOON'
        ).select_related('staff_member', 'staff_member__unit')[:10]
    ...
    context = {
        ...
        'care_homes': care_homes,
        'is_senior_leadership': request.user.can_view_all_homes,
    }
    return render(request, 'scheduling/compliance_dashboard_new.html', context)
