...
class RotaHealthScorer:
    ...
    def _score_compliance(self):
        ...
        # Note: duration_hours is a @property, so we calculate after retrieval
        all_shifts = self.shifts.filter(user__isnull=False).select_related('shift_type')
        long_shifts = [shift for shift in all_shifts if shift.duration_hours > 12]
        
        if long_shifts:
            count = len(long_shifts)
