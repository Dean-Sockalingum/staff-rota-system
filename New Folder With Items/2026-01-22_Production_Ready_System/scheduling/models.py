...
class Role(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        # ADMIN/superuser also gets full permissions
            ...
            # All other roles (SSCW, SCW, SCA) do NOT have management permissions
            self.is_management = False
            self.is_senior_management_team = False
            if self.name == 'SSCW':
                self.permission_level = 'MOST'
            else:
                self.permission_level = 'LIMITED'
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    @property
    def get_accessible_care_homes(self):
        """
        Get list of care homes this user can access.
        Senior leadership gets all homes, regular staff get their assigned home only.
        """
        if self.can_view_all_homes:
            from .models import CareHome
            return CareHome.objects.all()
        elif self.assigned_care_home:
            return [self.assigned_care_home]
        else:
            return []
