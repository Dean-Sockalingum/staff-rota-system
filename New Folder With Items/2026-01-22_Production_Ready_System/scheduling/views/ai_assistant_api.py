...
class ReportGenerator:
    ...
    def generate_shift_coverage_report(date_str=None, care_home_name=None):
        """Generate shift coverage report for a specific date and optionally a specific care home"""
        if date_str:
            try:
        ...
        shifts_qs = Shift.objects.filter(date=target_date).select_related('user', 'shift_type', 'unit', 'unit__care_home')
        ...
        for shift in shifts:
            ...
            if unit_name not in coverage_by_unit:
                coverage_by_unit[unit_name] = {'day': 0, 'night': 0, 'staff': []}
            ...
            if shift.user:
                ...
                coverage_by_unit[unit_name]['staff'].append(staff_info)
        ...
        # Build summary
        if care_home_name:
            summary = f"**{care_home_name.title()} Staffing on {target_date.strftime('%A, %d %B %Y')}:**\n"
            summary += f"{len(shifts)} staff members scheduled across {len(coverage_by_unit)} units"
        else:
            summary = f"On {target_date.strftime('%A, %d %B %Y')}: {len(shifts)} shifts scheduled across {len(coverage_by_unit)} units."
    ...
    def interpret_query(query):
        ...
        Returns report data ONLY for queries that are clearly asking for reports/analytics.
        Most conversational queries should return None so they fall through to the knowledge base.
        """
        query_lower = query.lower()
        
        # Care home names for pattern matching
        care_home_patterns = ['orchard grove', 'meadowburn', 'hawthorn house', 'riverside', 'victoria gardens', 'og', 'mb', 'hh', 'rs', 'vg']
        
        # SPECIFIC CARE HOME QUERIES - These need actual data, not generic help
        # Pattern: "[home name] staffing [today/tonight/date]" or "[home name] shifts"
        is_care_home_query = any(home in query_lower for home in care_home_patterns)
        is_staffing_query = any(word in query_lower for word in ['staffing', 'staff today', 'working', 'shifts', 'rota', 'coverage', 'on duty'])
        is_time_specific = any(word in query_lower for word in ['today', 'tonight', 'tomorrow', 'now', 'currently'])
        
        if is_care_home_query and is_staffing_query:
            # This is asking for actual shift data for a specific care home
            date_str = None
            if 'tomorrow' in query_lower:
                date_str = (timezone.now().date() + timedelta(days=1)).isoformat()
            elif is_time_specific or 'today' in query_lower:
                date_str = timezone.now().date().isoformat()
            
            # Extract care home name
            care_home_name = None
            for home in ['orchard grove', 'meadowburn', 'hawthorn house', 'riverside', 'victoria gardens']:
        ...
        # Generic "who is working today" without specific home - also needs actual data
        if any(phrase in query_lower for phrase in ['who is working today', 'who is on duty', 'shifts today', 'rota today', 'schedule today']) and not any(name in query_lower for name in ['jane', 'john', 'smith']):
            date_str = None
            if 'tomorrow' in query_lower:
                date_str = (timezone.now().date() + timedelta(days=1)).isoformat()
        ...
        # ML Forecasting queries (explicit forecast/prediction requests only)
        if any(phrase in query_lower for phrase in ['staffing forecast', 'demand forecast', 'predict staffing', 'prediction for']):
            days = 7
            if 'tomorrow' in query_lower:
                days = 1
        ...
        # Staffing shortage queries (explicit shortage/understaffed mentions only)
        ...
        # Staffing summary queries (explicit count/levels requests only)
        if any(phrase in query_lower for phrase in ['how many staff', 'total staff', 'staff count', 'staffing levels']):
            return {'type': 'staffing_summary', 'data': ReportGenerator.generate_staffing_summary()}
        
        # Sickness report queries (ONLY when asking for "report" or "show ... sick")
        if any(phrase in query_lower for phrase in ['sickness report', 'sickness absence report', 'who is off sick', 'show.*sick']):
            days = 7
        ...
        # Incident report queries (explicit report requests only)
        if any(phrase in query_lower for phrase in ['incident report', 'show.*incident', 'recent incidents', 'incident.*this week']):
            days = 7
        ...
        # Leave summary report queries (explicit summary/report requests only)
        if any(phrase in query_lower for phrase in ['leave requests', 'annual leave pending', 'leave approvals']):
        ...
        # NOTE: Removed "who is working", "shift coverage", and other conversational queries
        # These should be handled by the knowledge base for more natural responses
    ...
    try:
        ...
        if report_result:
            ...
            if report_type == 'staffing_summary':
                ...
                # Show detailed staff list for shift coverage
                if report_data.get('staff_list'):
                    answer += "\n\n**Staff on Duty:**\n"
                    # Group by unit for better readability
                    for unit, coverage in report_data['by_unit'].items():
                        answer += f"\n**{unit}**: {coverage['day']} day shift | {coverage['night']} night shift\n"
                        for staff in coverage.get('staff', []):
                            answer += f"  • {staff['name']} ({staff['role']}) - {staff['shift']}\n"
                else:
                    # Fallback to unit summary if no staff data
                    answer += "\n\n**Coverage by Unit:**\n"
                    for unit, coverage in report_data['by_unit'].items():
                        answer += f"• {unit}: Day {coverage.get('day', 0)} | Night {coverage.get('night', 0)}\n"
                
                if report_data['total_shifts'] == 0:
