...
def staff_guidance(request):
    ...
    # BASE_DIR is /path/to/Staff_Rota_Backups, docs is at BASE_DIR/docs
    docs_path = Path(settings.BASE_DIR) / 'docs' / 'staff_guidance'
...
def staff_home_units(request):
    """Display staff home unit assignments with pagination"""
    from django.core.paginator import Paginator
    
    # Get all active staff with their home units
    staff_list = User.objects.filter(
        is_active=True,
        home_unit__isnull=False
    ).select_related('home_unit', 'role').order_by('home_unit__name', 'last_name', 'first_name')
    
    # Pagination
    paginator = Paginator(staff_list, 50)  # Show 50 staff per page
    ...
    return render(request, 'scheduling/staff_home_units.html', context)
...
def audit_trail_view(request):
    ...
    # Handle case where user doesn't have a care home assigned
    if not care_home:
        messages.error(request, 'You must be assigned to a care home to view the audit trail.')
        return redirect('dashboard')
    ...
    # Get filter options - users from the same care home
    users = User.objects.filter(unit__care_home=care_home).distinct()
